...
import { FormKey, AnimSet as AnyAnimSet, catalog } from "../game/catalog";
...
export default function Tamagotchi({
  ...
  // pool of child forms and mapping to adults
  ...
  const CATA_COUNT_KEY = "wg_cata_count_v4";
  ...
  const urls = useMemo(() => {
    ...
    // also add egg frames (для автоскейла)
  ...
  // offline catch-up: stats/illness/catastrophes + age; anti-backwards-time
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      // catastrophe: after 2 days, once/min roll; first 3 are guaranteed
  ...
  const act = {
    feed: () => { if (deadRef.current) return;
      setStats(s => clampStats({ ...s, hunger: s.hunger + 0.25, happiness: s.happiness + 0.05 }));
      if (Math.random() < 0.7) spawnPoop();
    },
    play: () => { if (deadRef.current) return;
      setStats(s => clampStats({ ...s, happiness: s.happiness + 0.2, health: Math.min(1, s.health + 0.03) }));
    },
    clean: () => { if (deadRef.current) return;
      setPoops([]); setStats(s => clampStats({ ...s, cleanliness: Math.max(s.cleanliness, 0.92), happiness: s.happiness + 0.02 }));
    },
    heal: () => { if (deadRef.current || !canHeal) return;
      setIsSick(false);
  ...
  const spendLifeToRevive = () => {
    ...
    setIsDead(false);
    setDeathReason(null);
    setStats(s => clampStats({ cleanliness: Math.max(s.cleanliness, 0.7), hunger: 0.4, happiness: Math.max(s.happiness, 0.5), health: Math.max(s.health, 0.6) }));
    setPoops([]); setIsSick(false); setCatastrophe(null);
  };
  ...
  useEffect(() => {
    ...
    return () => { alive = false; cancelAnimationFrame((window as any).__wg_raf || 0); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  function startLoop(images: Record<string, HTMLImageElement>) {
    ...
    const resize = () => {
      ...
      const containerW = wrap.clientWidth || LOGICAL_W, containerH = CANVAS_H;
      const target = LOGICAL_W / LOGICAL_H, box = containerW / containerH;
      let cssW = containerW, cssH = containerH;
      if (box > target) cssW = Math.round(containerH * target); else cssH = Math.round(containerW / target);
      canvas.style.width = cssW + "px"; canvas.style.height = cssH + "px";
      canvas.width = cssW * dpr; canvas.height = cssH * dpr;
      ctx.setTransform(1,0,0,1,0,0); ctx.scale((cssW*dpr)/LOGICAL_W, (cssH*dpr)/LOGICAL_H);
      (ctx as any).imageSmoothingEnabled = false;
    };
    ...
    let dir: 1 | -1 = 1; let x = 40;
    let last = performance.now(); let frameTimer = 0;

    // target world height from egg frame (for autoscale)
    ...
    const loop = (ts: number) => {
      ...
      if (bg) {
        ...
        const dw = Math.floor(bg.width * scaleBG), dh = Math.floor(bg.height * scaleBG);
        const dx = Math.floor((LOGICAL_W - dw) / 2), dy = Math.floor((LOGICAL_H - dh) / 2);
        ctx.drawImage(bg, dx, dy, dw, dh);
      }

      // avatar (top-right): uses sick if available
      const now = Date.now();
      const sleepingNow = isSleepingAt(now);
      const avatarKey: AnimKey = (() => {
      ...
      const avFrames = (def[avatarKey] ?? def.idle ?? def.walk ?? []) as string[];
      const avSrc = avFrames[0];
      if (avSrc && images[avSrc]) {
        const av = images[avSrc];
        const aw = 40, ah = 40, padX = 10, padY = 4, ax = LOGICAL_W - padX - aw, ay = padY;
        (ctx as any).imageSmoothingEnabled = false;
      ...
      if (curPoops.length) {
        for (const p of curPoops) {
          const img = images[p.src]; const px = Math.round(p.x); const py = Math.round(LOGICAL_H - BASE_GROUND - 6);
          if (img) ctx.drawImage(img, px, py - 12, 12, 12); else { ctx.font = "10px monospace"; ctx.fillText("💩", px, py); }
        }
      }
      ...
      const scale = targetH / Math.max(1, rawH);              // <-- AUTOSCALE to egg height
      ...
      if (!deadRef.current && !sleepingNow) {
        ...
        if (x < minX) { x = minX; dir = 1; } else if (x > maxX) { x = maxX; dir = -1; }
      }
      ...
      if (deadRef.current) {
        ...
        if (deadImg) {
          const w = Math.round(deadImg.width * scale), h = Math.round(deadImg.height * scale);
          const ix = Math.round((LOGICAL_W - w) / 2), iy = Math.round(BASELINE - h);
  ...
  return (
    <div style={{ width: "min(92vw, 100%)", maxWidth: MAX_W, margin: "0 auto" }}>
      <div ref={wrapRef} style={{ width: "100%", height: CANVAS_H, position: "relative", imageRendering: "pixelated", overflow: "hidden", background: "transparent", borderRadius: 12, margin: "0 auto" }}>
      ...
      <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", opacity: isDead ? 0.5 : 1, pointerEvents: isDead ? ("none" as const) : ("auto" as const) }}>
      ...
      <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", justifyContent: "center" }}>
        ...
        {!sleepLocked ? (
          <button className="btn" onClick={() => { localStorage.setItem(SLEEP_FROM_KEY, sleepStart); localStorage.setItem(SLEEP_TO_KEY, wakeTime); localStorage.setItem(SLEEP_LOCK_KEY, "1"); setSleepLocked(true); setUseAutoTime(false); }}>
            Save & lock
          </button>
...
function drawBanner(ctx: CanvasRenderingContext2D, width: number, text: string) {
  ...
  const x = Math.round((width - w) / 2); const y = 18;
  ctx.fillStyle = "rgba(0,0,0,0.55)"; ctx.fillRect(x, 6, w, 18);
  ctx.fillStyle = "white"; ctx.fillText(text, x + pad, y); ctx.restore();
}
function clampDt(ms: number) { if (!Number.isFinite(ms) || ms < 0) return 0; return Math.min(ms, 10 * 60 * 1000); }
async function loadImageSafe(src: string): Promise<{ src: string; img: HTMLImageElement } | null> {
  return new Promise((resolve) => { try { const img = new Image(); img.crossOrigin = "anonymous"; img.onload = () => resolve({ src, img }); img.onerror = () => resolve(null); img.src = src; } catch { resolve(null); } });
}

// offline simulator (minute-by-minute)
function simulateOffline(args: {
  ...
  let s = { ...args.startStats }; let sick = args.startSick; let cataIncrements = 0;
  ...
  for (let i = 0; i < args.minutes; i++) {
    ...
    if (!sleeping) {
      ...
      if (!sick) {
        const p = 0.02 + 0.3 * 0.3 + 0.2 * (1 - s.cleanliness);
        if (Math.random() < p * 0.03 * 60) sick = true;
      } else {
        if (Math.random() < 0.015 * 60) sick = false;
      }
    }
    if (s.hunger <= 0 || s.health <= 0) break;
  }
